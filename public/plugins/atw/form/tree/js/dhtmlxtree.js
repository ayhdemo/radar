...
dhtmlXTreeObject.prototype._parseItem = function(c, temp, preNode, befNode) {
    var id;
///////////////////////////////////////////////////////////////
    var node = new $.fn.base.Xml.Node({node:c.d});
    this.extendAttributes[c.get("id")+""] = node.getAttributes();
    this.extendAttributesJson[c.get("id")+""] = node.getAttributesJson();
    ///////////////////////////////////////////////////////////////
...
dhtmlXTreeObject.prototype._createItem = function(acheck, itemObject, mode) {
    ...
    //add by cqb
    var textnode = document.createTextNode(itemObject.label);
	itemObject.span.appendChild(textnode);
...
dhtmlXTreeObject.prototype._loadDynXML = function(id, src, cback) {
    src = src || this.XMLsource;
    var sn = (new Date()).valueOf();
    this._ld_id = id;
    ///////////////////////////////////////////////////////////////////////
    var atts = this.extendAttributes[id+""];
    var extendParams = new Array();
    if(typeof(atts) != "undefined"){
        for(var i=0,num = atts.length; i<num; i++){
            extendParams.push(atts[i].attributeName + "=" + encodeURI(encodeURI(atts[i].attributeValue)));
        }
    }
    ///////////////////////////////////////////////////////////////////////
    if (this.Wa == "function") {
        ...
        this.loadXML(src + this._escape(id), cback);
    else if (this.Wa == "xmlname")
        this.loadXML(src + this._escape(id) + ".xml?uid=" + sn, cback);
    else
        this.loadXML(src + getUrlSymbol(src) + "uid=" + sn + "&id=" + this._escape(id), cback)
};
