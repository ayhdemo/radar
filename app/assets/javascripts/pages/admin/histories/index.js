    ...
    $tbl_his.bootstrapTable({
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function (value, row) {
                    return [
                        mylib.account_url_template(row.from_account_name, row.from_chain_type),
                        mylib.account_url_template(row.monitor_account_name, row.from_chain_type),
                    ].join(' -> ')
                },
                valign: 'middle'
            },
            {
                ...
                formatter: function (value, row) {
                    return [
                        row.in_amount,
                        row.in_asset_alias.length && row.in_asset_alias || row.in_asset_name
                    ].join(' ')
                }
            },
            {
            ...
            },
            {
                ...
                title: '转入块号',
                formatter: function (idx, row) {
                    return mylib.block_url_template(row.in_block_num, row.from_chain_type)
                },
                valign: 'middle'
            },
            {
                ...
                formatter: function (index, row) {
                    return mylib.dateConvert(row.in_block_time)
                },
            },
            {
                title: '转入交易ID',
                ...
                formatter: function (value, row) {
                    return [
                        mylib.tx_url_template(row.in_trx_id, row.from_chain_type, true)
                    ].join('')
                },
            },
            {
                ...
                valign: 'middle',
                formatter: function (index, row) {
                    return mylib.dateConvert(row.out_block_time)
                },
            },
            {
                ...
                formatter: function (index, row) {
                    ...
                    switch (row.process_status) {
                        case 'new':
                            cls = 'text-my-light-blue'; break;
                        case 'auto_out_confirmed':
                            cls = 'text-my-green'; break;
                        case 'auto_out_processing':
                        case 'auto_out_processed':
                            cls = 'text-my-blue'; break;
                        default:
                            cls = 'text-my-red';
                    }
                    ...
                    var status = process_status_trans[row.process_status] || row.process_status;
            ...
            },
            {
                ...
                formatter: (idx, row) => {
                    ...
                    let manual_comment_exists = [
                        '<a class="btn default btn-xs green manual-comment">',
                        '<i class="fa fa-edit"></i> 已标记',
                        '</a>',
                    ].join('');
